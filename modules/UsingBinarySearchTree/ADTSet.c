...
struct set_node {
	...
	int left_subtree_size;     ////Το μεγεθος του αριστερου υποδεντρου του κομβου
...
static SetNode node_match_pos(SetNode node, int pos){      ////Συναρτηση που βρισκει το κομβο της θεσης pos του set ,ξεκινωντας απο την ριζα node
	assert(pos>=0);

	if(pos==node->left_subtree_size){       ////Αν η θεση ειναι η ριζα
		return node;
	}else if(pos<node->left_subtree_size){      ////Αν η θεση ειναι μικροτερη απο το αρ.υποδεντρο της ριζας ,τοτε σημαινει οτι ειναι σε αυτο

		return node_match_pos(node->left,pos);
	}else{        ////Αν η θεση ειναι μεγαλυτερη,σημαινει οτι ειναι στο δεξ.υποδεντρο της ριζας και για να τη βρουμε πρεπει να αφαιρεσουμε απο αυτη left_subtree_size+1
		return node_match_pos(node->right,pos-(node->left_subtree_size+1));
	}
}
...
Pointer set_get_at(Set set,int pos){
	...
	SetNode node=node_match_pos(set->root,pos);      ////O κομβος που αντιστοιχει στην θεση

	return node->value;      ////Επιστρεφουμε τα περιχομενα του 
}
...
void set_set_at(Set set, int pos, Pointer value){
	...
	set_remove(set,set_get_at(set,pos));   ////Βρισκουμε το κομβο στη θεση pos και τον αφαιρουμε
	set_insert(set,value);         ////Εισαγουμε τη νεα τιμη στο set
}
