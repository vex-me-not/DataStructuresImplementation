...
void test_get_set_at(void) {
	Set s=set_create(compare_ints,free);     ////Δημιουργουμε ενα set

	for(int i=0;i<10;i++){       ////Εισαγουμε 10 στοιχεια σε αυτο
		int* ins=create_int(i);
		set_insert(s,ins);
		TEST_ASSERT(compare_ints(set_get_at(s,i),ins)==0);
	}
	
	int init_size=set_size(s);        ////Το αρχικο μεγεθος
	
	for(int i=2*set_size(s);i<3*set_size(s);i++){
		int* st=create_int(i);
		int set_pos=i-2*set_size(s);
		
		set_set_at(s,set_pos,st);
		TEST_ASSERT(set_size(s)==init_size);       ////Κατα τις κλησεις της set_at το μεγεθος του set δεν πρεπει να αλλαζει
	}
